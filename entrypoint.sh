...
if [ "$1" = 'mysqld' ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ "$MYSQL_DATABASE" ]; then
			echo "CREATE DATABASE IF NOT EXISTS \`$MYSQL_DATABASE\` ${MYSQL_CHARSET:+DEFAULT CHARACTER SET $MYSQL_CHARSET} ${MYSQL_COLLATION:+DEFAULT COLLATE $MYSQL_COLLATION} ;" >> "$tempSqlFile"
		fi
    ...
    echo "SET GLOBAL max_allowed_packet=10485760 ;" >> "$tempSqlFile" 
